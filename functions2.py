...
def book_rent(check_result):  # 장바구니에 있는 목록 출력 후 대여여부 선택하기
    ...
    # test_sql = '''
    #      SELECT book_list.book_num, book_list.lib_name, book_list.book_name, book_list.author, book_list.publisher, book_list.borrow
    #     FROM book_list, cart
    #     WHERE book_list.book_num LIKE CONCAT(%s) and book_list.book_num = cart.book_num and cart.cust_id = %s
    #     '''
    #
    #
    # if result == None:
    #     print("누군가의 장바구니에 담긴 도서이거나 이미 대여중인 도서입니다.")
    #     DB.disconnect()
    #     return
